...
export class PS2 {
    cpu: CPU;
    bus: BusConnector;
    wheel_movement!: number;
    enable_mouse_stream!: boolean;
    use_mouse!: boolean;
    have_mouse!: boolean;
    mouse_delta_x!: number;
    mouse_clicks!: number;
    have_keyboard!: boolean;
    enable_keyboard_stream!: boolean;
    next_is_mouse_command!: boolean;
    next_read_sample!: boolean;
    next_read_led!: boolean;
    next_handle_scan_code_set!: boolean;
    next_read_rate!: boolean;
    next_read_resolution!: boolean;
    kbd_buffer!: ByteQueue;
    last_port60_byte!: number;
    sample_rate!: number;
    mouse_detect_state!: number;
    mouse_id!: number;
    mouse_reset_workaround!: boolean;
    resolution!: number;
    scaling2!: boolean;
    last_mouse_packet!: number;
    mouse_buffer!: ByteQueue;
    next_byte_is_ready!: boolean;
    next_byte_is_aux!: boolean;
    command_register!: number;
    controller_output_port!: number;
    read_output_register!: boolean;
    read_command_register!: boolean;
    read_controller_output_port!: boolean;
    mouse_delta_y!: any;
    constructor(cpu: CPU, bus: BusConnector) {
        ...
        this.bus.register("keyboard-code", (code: any) => {
            this.kbd_send_code(code);
        }, this);

        this.bus.register("mouse-click", (data: any[]) => {
            this.mouse_send_click(data[0], data[1], data[2]);
        }, this);

        this.bus.register("mouse-delta", (data: any[]) => {
            this.mouse_send_delta(data[0], data[1]);
        }, this);

        this.bus.register("mouse-wheel", (data: number[]) => {
    ...
    mouse_send_delta(delta_x: number, delta_y: number) {
    ...
    mouse_send_click(left: number, middle: number, right: number) {
        if (!this.have_mouse || !this.use_mouse) {
            return;
        }
    ...
    send_mouse_packet(dx: number, dy: number) {
        var info_byte =
            (+ (dy < 0)) << 5 |
            (+ (dx < 0)) << 4 |
            1 << 3 |
    ...
    port60_write(write_byte: number) {
