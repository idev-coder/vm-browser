...
export class IDEDevice {
    private current_interface: any;
    private cpu: CPU;
    private master: IDEInterface;
    private slave: IDEInterface;
    private ata_port: any;
    private irq: any;
    private pci_id: any;
    private ata_port_high: any;
    private master_port: number;
    private pci_space: any[];
    private pci_bars: ({ size: number; } | undefined)[];
    private name: string;
    private device_control: number;
    private prdt_addr: number;
    private dma_status: number;
    private dma_command: number;
    constructor(cpu: CPU, master_buffer: any, slave_buffer: any, is_cd: boolean, nr: number, bus: BusConnector) {
        ...
        cpu.io.register_write(this.ata_port | 0, this, (data: any) => {
        ...
        cpu.io.register_write(this.ata_port | 1, this, (data: number) => {
        ...
        cpu.io.register_write(this.ata_port | 2, this, (data: number) => {
        ...
        cpu.io.register_write(this.ata_port | 3, this, (data: number) => {
        ...
        cpu.io.register_write(this.ata_port | 4, this, (data: number) => {
        ...
        cpu.io.register_write(this.ata_port | 5, this, (data: number) => {
        ...
        cpu.io.register_write(this.ata_port | 6, this, (data: number) => {
            var slave = data & 0x10;
            var mode = data & 0xE0;
        ...
        cpu.io.register_write(this.ata_port | 7, this, (data: any) => {
            dbg_log("lower irq", LOG_DISK);
            this.cpu.device_lower_irq(this.irq);
    ...
    dma_write_command(value: number) {
    ...
    dma_write_command8(value: number) {
...
class IDEInterface {
    ...
    write_dest: number;
    last_io_id: number;
    in_progress_io_ids: any;
    cancelled_io_ids: any;
    sectors_per_drq: any;

    constructor(device: any, cpu: CPU, buffer: any, is_cd: boolean, device_nr: number, interface_nr: any, bus: BusConnector) {
        this.device = device;
    ...
    atapi_read(cmd: any[] | Uint8Array<ArrayBuffer>) {
        // Note: Big Endian
        ...
        if (start >= this.buffer.byteLength) {
            ...
            this.read_buffer(start, byte_count, (data: any) => {
                //setTimeout(() => {
    ...
    atapi_read_dma(cmd: any[] | Uint8Array<ArrayBuffer>) {
        // Note: Big Endian
        var lba = cmd[2] << 24 | cmd[3] << 16 | cmd[4] << 8 | cmd[5];
        var count = cmd[7] << 8 | cmd[8];
        var flags = cmd[1];
    ...
    write_data_port(data: number, length: number) {
    ...
    write_data_port8(data: number) {
        this.write_data_port(data, 1);
    };

    write_data_port16(data: number) {
        this.write_data_port(data, 2);
    };

    write_data_port32(data: number) {
    ...
    ata_advance(cmd: number, sectors: any) {
    ...
    ata_read_sectors_dma(cmd: number) {
    ...
    ata_write_sectors_dma(cmd: number) {
    ...
    data_allocate_noclear(len: number) {
