...
function save_object(obj: any, saved_buffers: any): any
{
    ...
    if(obj.BYTES_PER_ELEMENT)
    {
        ...
        const constructor = obj.constructor.name.replace("bound ", "") as keyof typeof CONSTRUCTOR_TABLE;
...
function restore_buffers(obj: string | any[] | null, buffers: any[])
{
    if(typeof obj !== "object" || obj === null)
    {
        dbg_assert(typeof obj !== "function");
    ...
    return new (constructor as { new(buffer: ArrayBufferLike): any })(buffer);
}

/* @param {CPU} cpu */
export function save_state(cpu:CPU)
{
    var saved_buffers:any[] = [];
...
/* @param {CPU} cpu */
export function restore_state(cpu:CPU, state: any)
{
    state = new Uint8Array(state);

    function read_state_header(state: Uint8Array<any>, check_length: boolean)
    {
    ...
    function read_info_block(info_block_buffer: Uint8Array<any> | AllowSharedBufferSource | undefined)
    {
    ...
    else
    {
        ...
        const buffers = buffer_infos.map((buffer_info: { offset: number; length: any; }) => {
