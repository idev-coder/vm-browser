...
// For Types Only
import { CPU } from "./cpu";
import { ScreenAdapter } from "./browser/screen";
import { BusConnector } from "./bus";
import { DummyScreenAdapter } from "./browser/dummy_screen";
import { round_up_to_next_power_of_2, view } from "./lib";
...
export class VGAScreen {
    ...
    vga256_palette: Int32Array<ArrayBuffer>;
    latch_dword: number;
    svga_version: number;
    svga_width: number;
    svga_height: number;
    ...
    dac_state: number;
    dac_mask: number;
    dac_map: Uint8Array<ArrayBuffer>;
    ...
    image_data: any;
    vga_memory: Uint8Array<ArrayBuffer>;
    plane0: Uint8Array<ArrayBuffer>;
    plane1: Uint8Array<ArrayBuffer>;
    plane2: Uint8Array<ArrayBuffer>;
    plane3: Uint8Array<ArrayBuffer>;
    pixel_buffer: Uint8Array<ArrayBuffer>;
    dest_buffet_offset: number;
    constructor(cpu: CPU, bus: BusConnector, screen: ScreenAdapter | DummyScreenAdapter, vga_memory_size: number) {
        this.cpu = cpu;
        ...
        io.mmap_register(0xA0000, 0x20000,
            (addr: any) => this.vga_memory_read(addr),
            (addr: any, value: any) => this.vga_memory_write(addr, value),
        );
    ...
    vga_memory_write(addr: number, value: number) {
        if (this.svga_enabled) {
            // vbe banked mode (accessing svga memory through the regular vga memory range)
    ...
    vga_memory_write_graphical(addr: number, value: number) {
        var plane_dword:any;
     ...
     */
    apply_setreset(data_dword: number, enable_dword: number) {
        var setreset_dword = this.apply_expand(this.planar_setreset);
     ...
     * @see {@link http://www.osdever.net/FreeVGA/vga/graphreg.htm#03}
     */
    apply_logical(data_dword: number, latch_dword: number) {
     ...
     */
    apply_bitmask(data_dword: number, bitmask_dword: number) {
    ...
    vga_memory_write_text_mode(addr: number, value: number) {
    ...
    scan_line_to_screen_row(scan_line: number) {
     ...
     */
    set_size_text(cols_count: number, rows_count: number) {
    ...
    set_size_graphical(width: number, height: number, virtual_width: number, virtual_height: any, bpp: number) {
        dbg_assert(this.graphical_mode);
     ...
     */
    port3C9_write(color_byte: number) {
    ...
    port3D5_write16(register: number) {
    ...
    port1CE_write(value: number) {
        this.dispi_index = value;
    };

    port1CF_write(value: number) {
    ...
    set_font_bitmap(font_plane_dirty: boolean) {
