...
export class FloppyController {
    ...
    private cpu: CPU;
    private dma: any;
    private bytes_expecting: number;
    private receiving_command: Uint8Array<ArrayBuffer>;
    private receiving_index: number;
    private next_command: any;
    private response_data: Uint8Array<ArrayBuffer>;
    private response_index: number;
    private response_length: number;
    private status_reg0: number;
    private status_reg1: number;
    private status_reg2: number;
    private drive: number;
    private last_cylinder: number;
    private last_head: number;
    private last_sector: number;
    private dor: number;
    private dir: number;
    private fda_image: any;
    private fdb_image: any;
    private sectors_per_track: any;
    private number_of_heads: any;
    private number_of_cylinders: any;
    constructor(cpu: CPU, fda_image: any, fdb_image: any) {
    ...
    set_fda(fda_image: SyncBuffer | null | any) {
        var floppy_types = {
    ...
    port3F5_write(reg_byte: number) {
        ...
        if (this.bytes_expecting > 0) {
            ...
            switch (reg_byte) {
                ...
                case 0x45:
                case 0xC5:
                    this.next_command = function (args: any) { this.do_sector(true, args); };
                    this.bytes_expecting = 8;
                    break;
                case 0x06:
                case 0x46:
                case 0xC6:
                case 0xE6:
                    this.next_command = function (args: any) { this.do_sector(false, args); };
                    this.bytes_expecting = 8;
                    break;
                case 0x07:
    ...
    do_sector(is_write: boolean, args: any[]) {
        var head = args[2],
            cylinder = args[1],
            sector = args[3],
    ...
    done(args: number[], cylinder: number, head: number, sector: number, error: any) {
        if (error) {
