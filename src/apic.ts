...
import { LOG_APIC } from "../src/const";
import { h, int_log2 } from "./lib";
import { dbg_assert, dbg_log, dbg_trace } from "./log";
import { IOAPIC_CONFIG_MASKED, IOAPIC_DELIVERY_INIT, IOAPIC_DELIVERY_NMI, IOAPIC_DELIVERY_FIXED } from "./ioapic";
...
export class APIC {
    private cpu: CPU;
    private apic_id: number;
    private timer_divider: number;
    private timer_divider_shift: number;
    private timer_initial_count: number;
    private timer_current_count: number;
    private next_tick: any;
    private lvt_timer: number;
    private lvt_thermal_sensor: number;
    private lvt_perf_counter: number;
    private lvt_int0: number;
    private lvt_int1: number;
    private lvt_error: number;
    private tpr: number;
    private icr0: number;
    private icr1: number;
    private irr: Int32Array<ArrayBuffer>;
    private isr: Int32Array<ArrayBuffer>;
    private tmr: Int32Array<ArrayBuffer>;
    private spurious_vector: number;
    private destination_format: number;
    private local_destination: number;
    private error: number;
    private read_error: number;
    constructor(cpu: CPU) {
        /** @type {CPU} */
        this.cpu = cpu;
        ...
        cpu.io.mmap_register(APIC_ADDRESS, 0x100000,
            ...
            (addr: number) => this.read32(addr),
            (addr: number, value: number) => this.write32(addr, value)
        );
    }
    ...
    route(vector: number, mode: number, is_level: number, destination: number, destination_mode: number) {
        // TODO
        this.deliver(vector, mode, is_level);
    };

    deliver(vector: number, mode: any, is_level: any) {
    ...
    register_get_bit(v: Int32Array<ArrayBuffer> | number[], bit: number) {
        dbg_assert(bit >= 0 && bit < 256);
        return v[bit >> 5] >> (bit & 31) & 1;
    };

    register_set_bit(v: Int32Array<ArrayBuffer> | number[], bit: number) {
        dbg_assert(bit >= 0 && bit < 256);
        v[bit >> 5] |= 1 << (bit & 31);
    };

    register_clear_bit(v: Int32Array<ArrayBuffer> | number[], bit: number) {
        dbg_assert(bit >= 0 && bit < 256);
        v[bit >> 5] &= ~(1 << (bit & 31));
    };

    register_get_highest_bit(v: any[] | Int32Array<ArrayBuffer>) {
        for (var i = 7; i >= 0; i--) {
            var word = v[i];
