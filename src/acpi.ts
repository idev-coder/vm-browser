...
import { LOG_ACPI } from "../src/const";
import { h } from "./lib";
...
export class ACPI {
    private cpu: CPU;
    private timer_last_value: number;
    private status: number;
    private timer_imprecision_offset: number;
    private pm1_status: number;
    private pm1_enable: number;
    private last_timer: any;
    private gpe: Uint8Array<ArrayBuffer>;
    constructor(cpu: CPU) {
        /** @type {CPU} */
        this.cpu = cpu;
        ...
        io.register_write(0xB004, this, undefined, (value: number) => {
        ...
        // ACPI, gpe
        io.register_read(0xAFE0, this, () => {
            dbg_log("Read gpe#0", LOG_ACPI);
            return this.gpe[0];
        });
        io.register_read(0xAFE1, this, () => {
            dbg_log("Read gpe#1", LOG_ACPI);
            return this.gpe[1];
        });
        io.register_read(0xAFE2, this, () => {
            dbg_log("Read gpe#2", LOG_ACPI);
            return this.gpe[2];
        });
        io.register_read(0xAFE3, this, () => {
            dbg_log("Read gpe#3", LOG_ACPI);
            return this.gpe[3];
        });

        io.register_write(0xAFE0, this, (value: number) => {
            dbg_log("Write gpe#0: " + h(value), LOG_ACPI);
            this.gpe[0] = value;
        });
        io.register_write(0xAFE1, this, (value: number) => {
            dbg_log("Write gpe#1: " + h(value), LOG_ACPI);
            this.gpe[1] = value;
        });
        io.register_write(0xAFE2, this, (value: number) => {
            dbg_log("Write gpe#2: " + h(value), LOG_ACPI);
            this.gpe[2] = value;
        });
        io.register_write(0xAFE3, this, (value: number) => {
    ...
    set_state(state: Uint8Array<ArrayBuffer>[] | any[]) {
        this.status = state[0];
