import { dbg_log, LOG_LEVEL } from "./log";
...
const pad = function (size: number) {
    return {
        size,
        get: (offset: number) => -1,
    };
...
console.assert(Header.reduce((a: any, entry: any) => a + entry.size, 0) === 52);
...
console.assert(ProgramHeader.reduce((a: any, entry: any) => a + entry.size, 0) === 32);
...
console.assert(SectionHeader.reduce((a: any, entry: any) => a + entry.size, 0) === 40);


// From [{ name: type }, ...] to [{ name, type, size, get, set }, ...]
function create_struct(struct: any[]) {
    return struct.map(function (entry: any) {
...
export function read_elf(buffer: any) {
...
function read_struct(view: any, Struct: any) {
    const result: any = {};
    let offset: number = 0;
...
function read_structs(view: any, Struct: any, count: any): [any[], number] {
    const result: any[] = [];
    let offset = 0;
...
/** @param {number=} length */
function view_slice(view: any, offset: any, length?: any) {
