...
function translate_mac_address(packet: any, search_mac: any, replacement_mac: any) {
...
export function format_mac(mac: any) {
    return [
...
function dump_packet(packet: any, prefix: string) {
...
export class Ne2k {
    ...
    mar: Uint8Array<ArrayBuffer>;
    mac_address_in_state: any;
    rsar: any;
    pstart: any;
    pstop: any;
    curpg: any;
    boundary: any;
    constructor(cpu: CPU, bus: BusConnector, preserve_mac_from_state_image: boolean, mac_address_translation: boolean, id: number) {
        /** @const @type {CPU} */
        this.cpu = cpu;
        ...
        this.bus = bus;
        this.bus.register("net" + this.id + "-receive", (data: any) => {
            this.receive(data);
        }, this);
        ...
        io.register_write(this.port | E8390_CMD, this, (data_byte: number) => {
            this.cr = data_byte;
        ...
        io.register_write(this.port | NE_RESET, this, (data_byte: any) => {
            var pg = this.get_page();
            dbg_log("Write reset: " + h(data_byte, 2), LOG_NET);
            //this.isr &= ~ENISR_RESET;
        });

        io.register_read(this.port | EN0_STARTPG, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
                return this.pstart;
            }
        ...
        io.register_write(this.port | EN0_STARTPG, this, (data_byte: any) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_STOPPG, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
                return this.pstop;
            }
        ...
        io.register_write(this.port | EN0_STOPPG, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_ISR, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_TXCR, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
                this.txcr = data_byte;
        ...
        io.register_write(this.port | EN0_DCFG, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_RCNTLO, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_RCNTLO, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_RCNTHI, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_RCNTHI, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_RSARLO, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_RSARLO, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_RSARHI, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_RSARHI, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
                dbg_log("Write remote start address low: " + h(data_byte, 2), LOG_NET);
        ...
        io.register_write(this.port | EN0_IMR, this, (data_byte: any) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_BOUNDARY, this, (data_byte: any) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_TPSR, this, (data_byte: any) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_TCNTLO, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_TCNTLO, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_read(this.port | EN0_TCNTHI, this, () => {
            var pg = this.get_page();
            if (pg === 0) {
                dbg_assert(false, "TODO");
                return 0;
        ...
        io.register_write(this.port | EN0_TCNTHI, this, (data_byte: number) => {
            var pg = this.get_page();
            if (pg === 0) {
        ...
        io.register_write(this.port | EN0_RXCR, this, (data_byte: any) => {
            var pg = this.get_page();
            if (pg === 0) {
    ...
    data_port_write(data_byte: number) {
        if (NE2K_LOG_VERBOSE) {
    ...
    receive(data: ArrayLike<number> | any) {
